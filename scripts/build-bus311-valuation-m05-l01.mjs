...
const lessonDir = path.join(root, '02-VALUATION', 'M05');
const output = path.join(lessonDir, 'bus311-valuation-m05-l01-slides.html');
...
const slides = tvmM05L01Deck.slides.map((item, index) => {
...
const notes = tvmM05L01Deck.slides.map((item) => item.note);
const html = [
