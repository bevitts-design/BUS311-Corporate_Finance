...
const deckPath = path.join(root, 'FOUNDATIONS', 'M01', 'bus311-foundations-m01-l01-slides.html');
const legacyPilotPath = path.join(root, 'FOUNDATIONS', 'M01', 'bus311-foundations-m01-l01-canva-pilot-slides.html');
...
for (const filename of expectedImageAssets) {
  try {
    await fs.access(path.join(root, 'FOUNDATIONS', 'M01', 'assets', filename));
  } catch {
